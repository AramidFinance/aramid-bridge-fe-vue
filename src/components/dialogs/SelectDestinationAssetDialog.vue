...
const isOpen = computed(() => store.state.dialogSelectDestinationAssetIsOpen)
...
const fillInState = () => {
  ...
  if (
    ...
    //logger.debug('allowedRoutes', allowedRoutes)
...
onMounted(async () => {
  ...
  logger.debug('select destination asset dialog onmounted')
  fillInState()
})
...
<template>
  <div :class="store.state.dialogSelectDestinationAssetIsOpen ? '' : 'hidden'">
    ...
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="select-destination-asset-title"
      @keydown.esc="store.state.dialogSelectDestinationAssetIsOpen = false"
    >
      <div class="bg-gradient-brand drop-shadow-menu-default rounded-2xl p-3 flex flex-col h-full">
        <DialogTitle id="select-destination-asset-title">Select asset which you want to receive on other chain</DialogTitle>

        <!-- Search Bar -->
        <div class="mb-4">
          <label for="destination-asset-search" class="sr-only">Search tokens by name, symbol, or address</label>
          <input
            id="destination-asset-search"
            v-model="searchQuery"
            type="text"
